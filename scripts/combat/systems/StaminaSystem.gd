...
func apply_turn_regen(actor: CombatantRuntimeState, controls: Dictionary, status_defs: Dictionary) -> int:
	var regen_mod: int = actor.status_mod("sta_regen_mod", status_defs)
	var regen: int = int(controls.get("base_stamina_regen", 2)) + regen_mod
...
func recover(actor: CombatantRuntimeState, controls: Dictionary) -> int:
	var gain: int = int(controls.get("recover_bonus_stamina", 2))
